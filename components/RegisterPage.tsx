...
const RegisterPage: React.FC<RegisterPageProps> = ({ onRegister, switchToLogin, switchToLanding }) => {
            ...
            <div className="w-full max-w-md">
                ...
                <div className="bg-white rounded-xl border border-gray-200/80 shadow-lg p-8">
                    ...
                    <form onSubmit={handleSubmit}>
                         <div className="mb-4">
                            <label className="block text-sm font-semibold text-gray-600 mb-2">Tipo de Cuenta</label>
                            <div className="flex items-center p-1 bg-light rounded-lg">
                                <button type="button" onClick={() => setUserType('single_company')} className={`w-1/2 px-3 py-2 text-sm font-semibold rounded-md transition-all ${userType === 'single_company' ? 'bg-white text-primary shadow-sm' : 'text-gray-500'}`}>Cuenta Individual</button>
                                <button type="button" onClick={() => setUserType('professional_firm')} className={`w-1/2 px-3 py-2 text-sm font-semibold rounded-md transition-all ${userType === 'professional_firm' ? 'bg-white text-primary shadow-sm' : 'text-gray-500'}`}>Cuenta Profesional</button>
                            </div>
                        </div>
                        <div className="mb-4">
